...
- 用中文回复用户，代码注释可以用英文
- 保持回复简洁，避免冗长解释
- 你拥有跨会话持久记忆能力。使用 memory tool 来保存用户要求记住的信息、重要的项目上下文和决策，以及在需要时回忆之前保存的记忆
...
func buildSystemPrompt(skills *SkillRegistry, activeSkills []string, skillArgs map[string]string, memoryStore MemoryStore) string {
	...
	if memoryStore != nil {
		entries, err := memoryStore.List(nil, 50)
		if err == nil {
			sb.WriteString(buildMemoryPrompt(entries))
		}
	}
...
func RunAgentWithContent(deps *AgentDeps, sess *Session, userContent UserMessageContent, sse SSEWriter) error {
	...
	tools := ToolDefinitions(hasSkills, hasMemory)
	...
	for round := 0; round < maxRounds; round++ {
		...
		systemPrompt := buildSystemPrompt(deps.Skills, sess.ActiveSkills, sess.SkillArgs, deps.MemoryStore)

		// Handle long context: summarize old events if needed
		summary := sess.ConversationSummary
		summaryIdx := sess.SummaryUpToEventIndex
		eventThreshold := 40
		recentCount := 20
		if deps.Config != nil {
			if n, err := strconv.Atoi(deps.Config.MemoryEventThreshold); err == nil && n > 0 {
				eventThreshold = n
			}
			if n, err := strconv.Atoi(deps.Config.MemoryRecentCount); err == nil && n > 0 {
				recentCount = n
			}
		}

		if len(sess.Events) > eventThreshold {
			cutoff := len(sess.Events) - recentCount
			// Only re-summarize if at least 6 new events accumulated since last summary
			if cutoff > summaryIdx && (cutoff-summaryIdx) >= 6 {
				// Incremental: pass old summary + only new events since last summary
				newEvents := sess.Events[summaryIdx:cutoff]
				newSummary, err := summarizeEvents(newEvents, summary, deps.Claude)
				if err != nil {
					log.Printf("summarization failed, using existing summary: %v", err)
				} else if newSummary != "" {
					summary = newSummary
					summaryIdx = cutoff
					store.UpdateMemoryFields(sess.ID, sess.MemoryExtractedIndex, summary, summaryIdx)
		...
		messages := buildMessages(sess.Events, deps.ImageCache, summary, summaryIdx)

		// 3. Call Claude (streaming)
		var fullText string
		resp, err := deps.Claude.CallStream(systemPrompt, messages, tools, func(eventType string, data any) {
			switch eventType {
			case "text":
				text, _ := data.(string)
				fullText += text
		...
		for _, block := range resp.Content {
			...
			case "tool_use":
				...
				} else if block.Name == "memory" && deps.MemoryStore != nil {
					output, isError = executeMemoryTool(deps.MemoryStore, sess.ID, inputMap)
				} else {
		...
		if !hasToolUse {
			...
			if deps.MemoryStore != nil {
				freshSess, _ := store.Get(sess.ID)
				if freshSess != nil {
					go extractMemories(freshSess, store, deps.MemoryStore, deps.Claude)
				}
			}
			sse.WriteEvent("done", "{}")
			sse.Flush()
...
func executeSkillTool(deps *AgentDeps, sessionID string, input map[string]any) (string, bool) {
...
func buildMessages(events []Event, imgCache *ImageCache, summary string, summaryUpToIdx int) []llm.ClaudeMessage {
	...
	// If we have a summary, prepend it and only process recent events
	if summary != "" && summaryUpToIdx > 0 && summaryUpToIdx < len(events) {
		messages = append(messages, llm.ClaudeMessage{
			Role: "user",
			Content: []llm.ContentBlock{
				{Type: "text", Text: "[之前的对话摘要]\n" + summary},
			},
		})
		messages = append(messages, llm.ClaudeMessage{
			Role: "assistant",
			Content: []llm.ContentBlock{
				{Type: "text", Text: "好的，我已了解之前的对话上下文。请继续。"},
			},
		})
		events = events[summaryUpToIdx:]
	}
...
func extractUserMessageBlocks(content any, imgCache *ImageCache) []llm.ContentBlock {
	switch v := content.(type) {
	case string:
		if v == "" {
