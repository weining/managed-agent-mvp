...
| 字段 | 说明 | 默认值 |
|------|------|--------|
| `llm_provider` | LLM 提供商：`Codex`、`gemini`、`openai` | `Codex` |
| `llm_model` | 模型名称 | `Codex Sonnet 4.6` |
...
2. `harness.go:RunAgent` 加载会话事件，构建系统 prompt（基础 prompt + skill 摘要 + 激活 skill 的完整 prompt），调用 `LLMClient.CallStream`（由 `config.llm_provider` 决定实际实现：Codex/gemini/openai）
...
- `llm/Codex.go` — `ClaudeClient` 实现（Anthropic Messages API），序列化 `image` block 为 `source.type=base64` 格式
...
- `tools.go` — `ToolDefinitions`（Codex 工具 schema）与 `ExecuteTool`（分发至 Sandbox SDK）；含 `memory` 工具（save/recall/delete/list）
...
**图片视觉能力**：`user_message` 中 `IsImage=true` 的附件在 agent 循环前由 `prefetchSessionImages` 从沙箱下载，base64 编码后写入 `ImageCache`；`buildMessages` 时缓存命中则生成原生 `image` content block 传入 LLM，缓存 miss 降级为文本路径描述。三个 provider（Codex/OpenAI/Gemini）各自在序列化层处理 `image` block 格式差异，`CallStream` 接口签名不变。
