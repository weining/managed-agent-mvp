...
- 跨会话持久化记忆存储在 `data/memory.json`
- 混合管理模式：自动 LLM 提取 + 显式 `memory` 工具（save/recall/delete/list）
- 保存时采用三级去重：key 命中 -> 归一化内容命中 -> 新建条目
- 记忆会在每轮对话前注入系统 prompt
- 增量摘要会复用“旧摘要 + 新事件”，避免每次全量重算
- 最小增量门控为 6 条事件，减少过于频繁的摘要调用
...
| `memory_event_threshold` | 触发会话摘要的事件数量阈值           | `40`                               |
| `memory_recent_count`    | 滑动窗口中保留的最近事件数量         | `20`                               |
